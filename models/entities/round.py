...
class Round:
    ...
    def __init__(
        ...
        self.s_datetime = (
            datetime.fromisoformat(s_datetime)
            if isinstance(s_datetime, str)
            else s_datetime
        )
        self.e_datetime = (
            datetime.fromisoformat(e_datetime)
            if isinstance(e_datetime, str)
            else e_datetime
        )
    ...
    def e_datetime_display(self) -> str:
        if self.e_datetime:
            return self.e_datetime.strftime("%Y-%m-%d %H:%M")
        return "En cours"
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "e_datetime": (
                self.e_datetime.isoformat() if self.e_datetime else None
            ),
        }
