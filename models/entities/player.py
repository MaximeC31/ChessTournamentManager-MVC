...
class Player:
    ...
    def validate_birth_date(self, value: str) -> str:
        ...
        if len(value_splitted) != 3:
            raise ValueError(
                "Date de naissance invalide : format YYYY-MM-DD requis."
            )
        try:
            year, month, day = map(int, value_splitted)
            birth_date = datetime(year, month, day)
        except ValueError:
            raise ValueError("Date de naissance invalide : date incorrecte.")

        today = datetime.now()
        eighteen_years_ago = datetime(today.year - 18, today.month, today.day)
        if not (birth_date <= eighteen_years_ago):
            raise ValueError(
                "Date de naissance invalide : le joueur doit être majeur"
            )
    ...
    def validate_national_id(self, value: str) -> str:
        if not len(value) == 7:
            raise ValueError(
                f"Identifiant {value} invalide : "
                "longueur de 7 caractères requise."
            )

        if not value[:2].isalpha() or not value[:2].isupper():
            raise ValueError(
                f"Identifiant {value} invalide : "
                "les 2 premiers caractères invalides."
            )

        if not value[2:].isdigit():
            raise ValueError(
                f"Identifiant {value} invalide : "
                "les 5 derniers caractères invalides."
            )
